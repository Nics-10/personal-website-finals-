<template>
  <section id="hero" class="hero-section">
    <div class="paper-texture"></div>
    <div class="notebook-lines"></div>

    <div class="container hero-container">
      <div class="hero-content">
        
        <div class="profile-image-wrapper">
          <div class="tape tape-tl"></div>
          <div class="tape tape-br"></div>
          <div class="polaroid-frame">
            <img src="https://i.pinimg.com/736x/9e/66/d1/9e66d19de81af9b11623d47e69bf7577.jpg" alt="Profile" class="profile-image" />
            <p class="polaroid-tag">Neicha ✨</p>
          </div>
        </div>

        <div class="text-content">
          <h1 class="hero-title">Hello, I'm <span class="highlight">Neichaela Padilla</span></h1>
          <p class="hero-subtitle">Creative Developer & Designer</p>
          <p class="hero-description">
            I am a second-year student pursuing a career in the IT industry. One of my dreams is to work
            in the corporate world, where I can explore and improve myself in terms of both social and
            personal skills. I believe that this path will take me to many opportunities because as long
            as technology continues to evolve, IT professionals will always be in demand. I simply want
            to live my life to the fullest and be content with what I have.
          </p>

          <div class="social-buttons">
            <a href="https://www.instagram.com/neyyykkkaaaa" target="_blank" class="social-btn" aria-label="Instagram">
              <svg width="24" height="24" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
            </a>
            <a href="http://www.linkedin.com/in/neichaela-padilla-97ab3b322" target="_blank" class="social-btn" aria-label="LinkedIn">
              <svg width="24" height="24" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
            </a>
            <a href="mailto:neichaela10@gmail.com" class="social-btn" aria-label="Email">
              <svg width="24" height="24" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12.713l-11.985-9.713h23.97l-11.985 9.713zm0 2.574l-12-9.725v15.438h24v-15.438l-12 9.725z"/></svg>
            </a>
            <a href="https://www.facebook.com/share/1EHbLUszg1/?mibextid=wwXIfr" target="_blank" class="social-btn" aria-label="Facebook">
              <svg width="24" height="24" fill="currentColor" viewBox="0 0 24 24"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"/></svg>
            </a>
            <a href="https://github.com/Nics-10" target="_blank" class="social-btn" aria-label="GitHub">
              <svg width="24" height="24" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<style scoped>
@import url('https://fonts.googleapis.com/css2?family=Gochi+Hand&family=Architects_Daughter&display=swap');

/* --- Theme Variables --- */
.hero-section {
  --journal-bg: #fcfaf2;
  --journal-ink: #222;
  --journal-accent: #d35400;
  --journal-desc: #4f4f4f;
  --journal-highlight: #ffeaa7;
  --journal-polaroid: #ffffff;
  --journal-lines: rgba(0, 0, 0, 0.05);
  --journal-texture: 0.15;
  --journal-tape-1: rgba(194, 178, 128, 0.4);
  --journal-tape-2: rgba(173, 216, 230, 0.3);
  
  min-height: 100vh;
  display: flex;
  align-items: center;
  background-color: var(--journal-bg);
  color: var(--journal-ink);
  position: relative;
  overflow: hidden;
  padding: 60px 20px;
  transition: all 0.5s ease;
}

/* --- Dark Mode Logic --- */
:global(.dark-mode) .hero-section {
  --journal-bg: #1a1a1a;
  --journal-ink: #e0e0e0;
  --journal-accent: #ff7e2e;
  --journal-desc: #b0b0b0;
  --journal-highlight: #4d4321;
  --journal-polaroid: #2d2d2d;
  /* Darker/subtler lines for dark mode */
  --journal-lines: rgba(255, 255, 255, 0.02);
  --journal-texture: 0.05;
  --journal-tape-1: rgba(100, 90, 60, 0.3);
  --journal-tape-2: rgba(60, 80, 100, 0.2);
}

.paper-texture {
  position: absolute;
  inset: 0;
  opacity: var(--journal-texture);
  background-image: url("https://www.transparenttextures.com/patterns/natural-paper.png");
  pointer-events: none;
}

.notebook-lines {
  position: absolute;
  inset: 0;
  background-image: linear-gradient(var(--journal-lines) 1px, transparent 1px);
  background-size: 100% 30px;
  pointer-events: none;
}

.hero-container {
  max-width: 1100px;
  margin: 0 auto;
  z-index: 5;
}

.hero-content {
  display: flex;
  flex-direction: row;
  align-items: center;
  gap: 80px;
  text-align: left;
}

.profile-image-wrapper {
  position: relative;
  flex-shrink: 0;
  transform: rotate(-3deg);
  transition: transform 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275);
}

.profile-image-wrapper:hover {
  transform: rotate(0deg) scale(1.05);
}

.polaroid-frame {
  background: var(--journal-polaroid);
  padding: 15px 15px 45px 15px;
  box-shadow: 8px 12px 25px rgba(0,0,0,0.15);
  border: 1px solid rgba(0,0,0,0.05);
}

.profile-image {
  width: 280px;
  height: 320px;
  object-fit: cover;
  display: block;
}

.polaroid-tag {
  font-family: 'Gochi Hand', cursive;
  font-size: 1.8rem;
  color: var(--journal-ink);
  margin: 15px 0 0;
  text-align: center;
}

.tape {
  position: absolute;
  width: 100px;
  height: 35px;
  backdrop-filter: blur(1px);
  z-index: 10;
}

.tape-tl { 
  top: -15px; 
  left: -35px; 
  transform: rotate(-35deg); 
  background: var(--journal-tape-1);
}

.tape-br { 
  bottom: 15px; 
  right: -30px; 
  transform: rotate(-25deg); 
  background: var(--journal-tape-2); 
}

.hero-title {
  font-family: 'Architects Daughter', cursive;
  font-size: 3.5rem;
  margin-bottom: 10px;
  line-height: 1.2;
}

.highlight {
  position: relative;
  display: inline-block;
  z-index: 1;
}

.highlight::after {
  content: '';
  position: absolute;
  left: -5%;
  bottom: 10%;
  width: 110%;
  height: 40%;
  background: var(--journal-highlight);
  z-index: -1;
  transform: skewX(-15deg);
  animation: drawMarker 1s ease-out forwards;
}

.hero-subtitle {
  font-family: 'Gochi Hand', cursive;
  font-size: 2rem;
  color: var(--journal-accent);
  margin-bottom: 20px;
}

.hero-description {
  font-family: 'Architects Daughter', cursive;
  font-size: 1.2rem;
  color: var(--journal-desc);
  line-height: 1.7;
  max-width: 620px;
}

.social-buttons {
  margin-top: 40px;
  display: flex;
  gap: 20px;
}

.social-btn {
  width: 50px;
  height: 50px;
  display: flex;
  align-items: center;
  justify-content: center;
  color: var(--journal-ink);
  border: 2px solid var(--journal-ink);
  border-radius: 50% 45% 55% 48% / 48% 52% 45% 55%;
  transition: all 0.3s ease;
}

.social-btn:hover {
  transform: scale(1.1) rotate(5deg);
  background: var(--journal-ink);
  color: var(--journal-bg);
}

@keyframes drawMarker {
  from { width: 0; }
  to { width: 110%; }
}

@media (max-width: 850px) {
  .hero-content {
    flex-direction: column;
    text-align: center;
    gap: 40px;
  }
  .hero-description { margin: 0 auto; }
  .social-buttons { justify-content: center; }
  .hero-title { font-size: 2.5rem; }
}
</style>